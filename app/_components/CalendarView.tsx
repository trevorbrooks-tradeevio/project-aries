...
import type { CalendarEvents } from "../_lib/types";
...
export function CalendarView({ events }: { events: CalendarEvents }) {
      ...
      {!connected ? (
        <div className="cal-empty">
          ...
          <div className="ce-btns">
            ...
            <button className="btn btn-red" onClick={() => setConnected(true)} type="button"><Icons.Plus size={15} />Connect Outlook</button>
          </div>
        </div>
      ) : (
        <>
          <div className="cal-toolbar">
            ...
            <div className="legend">
              ...
              <span className="li"><span className="sw outlook" />Outlook</span>
              ...
              {agenda.map(([date, evs]) =>
                evs.map((e, i) => (
                  <div key={date + i} className="task-row" style={{ gridTemplateColumns: "auto 1fr auto", cursor: "default" }}>
                    ...
                    <span className={"chip " + (e.src === "google" ? "progress" : "open")}><span className="dot" style={{ background: e.src === "google" ? "var(--accent)" : "#2f6fd0" }} />{e.time}</span>
                  </div>
                ))
              )}
            </div>
